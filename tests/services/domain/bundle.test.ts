...
import { readConcept, updateConcept } from "@/main/services/domain/concept";
...
describe("tree display titles", () => {
  ...
  it("exposes the frontmatter title of a file", async () => {
    await write("a.md", "---\ntitle: Alpha\n---\n\nBody.\n");
    const tree = await listBundleTree();
    const a = (tree.children ?? []).find((c) => c.name === "a.md");
    expect(a?.title).toBe("Alpha");
  });

  it("falls back to the file name without extension", async () => {
    await touch("plain.md");
    const tree = await listBundleTree();
    const node = (tree.children ?? []).find((c) => c.name === "plain.md");
    expect(node?.title).toBe("plain");
  });

  it("uses the index.md title as the directory title", async () => {
    await write("docs/index.md", "---\ntype: Index\ntitle: Documents\n---\n");
    const tree = await listBundleTree();
    const docs = (tree.children ?? []).find((c) => c.name === "docs");
    expect(docs?.title).toBe("Documents");
  });

  it("falls back to the directory name when index.md is missing", async () => {
    await touch("bare/x.md");
    const tree = await listBundleTree();
    const bare = (tree.children ?? []).find((c) => c.name === "bare");
