...
describe("updateLinks", () => {
  it("rewrites inline links pointing at a renamed file", async () => {
  ...
  it("ignores bare paths that are not link-shaped (e.g. frontmatter)", async () => {
    await write("doc.md", "---\nrelated: /x.md\n---\n\nBody [x](/x.md).\n");

    await updateLinks({ root, from: "/x.md", to: "/y.md" });

    const out = await read("doc.md");
    expect(out).toContain("related: /x.md");
    expect(out).toContain("Body [x](/y.md).");
  });
...
describe("deleteLinks", () => {
  it("replaces links to a deleted file with their label text", async () => {
